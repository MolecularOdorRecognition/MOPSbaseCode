"""
Step 1 :: RAW DATA: NN Hidden Layer Size Variation
...
Required Directories:
    - CSVLogger   (logs all epoch information per epoch)
    - report      (stores Graph visualizations)
    - results     (final report generation of this run)
"""
import numpy
import keras
import pandas
...
del(X)
del(Y)
# deletion of data X, Y so as to free up valuable space
...
for size in range(15, 400):
    ...
    # logging of data
    printData(size=size, nBatch=nBatch,history=history, learnR=learn, drop=drop,fileName="results/results.csv") #"+str(size)+"
